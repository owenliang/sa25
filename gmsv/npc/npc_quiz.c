	...
	switch( num) {
	  case 0:
		...
		if(NPC_Util_GetStrFromStrWithDelim( argstr,"StartMsg",token, sizeof( token)) == NULL){
			print("%s��StartMsg������", CHAR_getChar(meindex,CHAR_NAME));
			return;
		}
		...
		if(entryflg == FALSE){
			...
			if(NPC_Util_GetStrFromStrWithDelim( argstr,"EntryItem",buf2, sizeof( buf2)) != NULL){
				if(NPC_EntryItemDel( talker, buf2)	==FALSE){
					print("����û����ʧ(%s)",CHAR_getChar(meindex,CHAR_NAME));
					return;
				}
			}
			if(NPC_Util_GetStrFromStrWithDelim( argstr,"EntryStone",buf3, sizeof( buf3)) != NULL){
				if(NPC_EntryStoneDel( talker, buf3)	==FALSE){
					print("��Ǯû����ʧ(%s)",CHAR_getChar(meindex,CHAR_NAME));
					return;
				}
			}
	  ...
	  case 3:
		...
		{
			print("%s��ItemFullMsg������", CHAR_getChar(meindex,CHAR_NAME));
			return;
		}
...
//				print("\n %s %d����",CHAR_getChar(talker,CHAR_NAME),(int)PLAYER.ptr);
...
BOOL NPC_PlayerCheck(int meindex,int talker)
	...
	if((ptr =(int *) calloc( 1,sizeof(struct pl)))  == NULL){
        print("�������޷�ȷ��%s",CHAR_getChar(meindex,CHAR_NAME));
    	return FALSE;
	}

    /*-- ���̼���������ë����������??-*/
    Player.ptr = ptr;
    Player.talkerindex = talker;	//npc���̼������͵�???
	Player.quizno = 0;		//�ػ���  ����
	Player.answer = 0;		//��ݩ��
	Player.ansno = 0;		//������  įë����������  ����ë�����±������׶˱�����)
...
int *NPC_GetQuestion(int meindex)
{
	...
	{
		/* BUGFIX: 原代码使用 VLA (int tmp_tbl[j+1]) 并返回其地址,
		 * 函数返回后该栈帧即失效,调用方得到悬空指针。
		 * 改为堆分配, 由调用方在使用完毕后释放。
		 * 注意: 调用方 (NPC_QuizInit / NPC_Quiz_selectWindow) 需配对 free。*/
		int *tmp_tbl = (int*)malloc( sizeof(int) * (j+1) );
		if( tmp_tbl == NULL ) {
			print("NPC_GetQuestion: malloc %d bytes failed\n", (int)(sizeof(int)*(j+1)));
			return NULL;
		}
		tmp_tbl[0] = j+1;
				...
				if( ITEM_CHECKINDEX(itemindex) ){
					...
					if(itemno==id){
						...
						LogItem(
							...
							itemindex,
#else
							ITEM_getInt( itemindex, ITEM_ID),  /* ʧ�̷�ة?į */
#endif
							"QuizDelItem(����->�����ĵ���)",
							CHAR_getInt( talker, CHAR_FLOOR),
							CHAR_getInt( talker, CHAR_X ),
